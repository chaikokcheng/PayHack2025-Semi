...
class Transaction(db.Model):
    ...
    original_amount = db.Column(db.Numeric(15, 2), nullable=True)  # Before FX conversion
    ...
    payment_method = db.Column(db.String(50), nullable=False)  # qr, nfc, online, etc.
    payment_rail = db.Column(db.String(50), nullable=False)  # duitnow, paynow, boost, tng
    status = db.Column(db.String(20), default='pending')  # pending, processing, completed, failed, refunded
    qr_code_id = db.Column(UUID(as_uuid=True), db.ForeignKey('qr_codes.id'), nullable=True)
    transaction_metadata = db.Column(JSON, default=dict)  # Additional transaction data
    ...
    # Relationships
    plugin_logs = db.relationship('PluginLog', backref='transaction', lazy='dynamic', cascade='all, delete-orphan')
